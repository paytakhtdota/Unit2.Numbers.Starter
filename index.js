...
class Numbers {
  ...
  odds() {
    ...
    let oddsNumbers = this.data.filter(item => { return item % 2 === 1 });
    return oddsNumbers;
  }
  evens() {
    //return the even numbers in data
    let evenNumbers = this.data.filter(item => { return item % 2 == 0 });
    return evenNumbers;
  }
  sum() {
  ...
  greaterThan(target) {
    ...
    let greaterThanTarget = this.data.filter(item => { return item > target });
...
console.log("The numbers greater than targert number :",n1.greaterThan(3)); //returns numbers greater than another number
const countHowMany = n1.howMany(3);
console.log("We have :",countHowMany[0], "count of the target number: (",countHowMany[1] , ") in the prompt input."); //return the count of a specific number
